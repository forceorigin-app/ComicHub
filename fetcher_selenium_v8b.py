    ...
    def get_images(self, chapter_url: str) -> list:
        ...
        try:
            ...
            while page_num < max_pages:
                ...
                # 检查是否有"下一页"按钮
                try:
                    next_button = self.driver.find_element(By.CSS_SELECTOR, '[data-action="next"]')
                    next_button.click()
                    time.sleep(2.5)  # 等待下一页加载
                    page_num += 1
                except:
                    logger.info(f"没有更多页面了，共获取 {len(all_images)} 张图片")
                    break
